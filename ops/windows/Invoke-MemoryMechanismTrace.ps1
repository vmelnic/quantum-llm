param(
    ...
    [string]$OutputName = "memory-mechanism-trace-v1",
    [int]$InDistributionCases = 2,
...
if ($DatasetName -notmatch '^[A-Za-z0-9._-]+$' -or
    ...
    throw "Invalid Memory Mechanism Trace parameters"
}
...
$output = Join-Path $outputRoot "trace.json"
...
foreach ($required in @(
    ...
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Memory Mechanism Trace artifact is missing: $required"
    }
}
...
if ($freeVramMiB -lt $MinimumFreeVramMiB) {
    throw "Memory Mechanism Trace requires $MinimumFreeVramMiB MiB free VRAM; available=$freeVramMiB"
}
...
$script = Join-Path $script:RepoRoot "experiments\memory_expert\mechanism_trace.py"
