param(
    ...
    [string]$OutputName = "memory-mechanism-intervene-v1",
    [int]$InDistributionCases = 8,
...
if ($DatasetName -notmatch '^[A-Za-z0-9._-]+$' -or
    ...
    $InDistributionCases -lt 1 -or $OutOfDistributionCases -lt 1 -or
    $MaximumNewTokens -lt 1 -or $MinimumFreeVramMiB -lt 1) {
    throw "Invalid Memory Mechanism Intervene parameters"
}
...
$output = Join-Path $outputRoot "intervention.json"
...
foreach ($required in @(
    ...
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Memory Mechanism Intervene artifact is missing: $required"
    }
}
...
if ($freeVramMiB -lt $MinimumFreeVramMiB) {
    throw "Memory Mechanism Intervene requires $MinimumFreeVramMiB MiB free VRAM; available=$freeVramMiB"
}
...
$script = Join-Path $script:RepoRoot "experiments\memory_expert\mechanism_intervene.py"
& $python $script --checkpoint $checkpointPath --corpus $corpusPath `
    --ingest $ingest --questions $questions --output $output --device cuda `
    --id-cases $InDistributionCases `
    --ood-cases $OutOfDistributionCases --maximum-new-tokens $MaximumNewTokens
$exitCode = $LASTEXITCODE

[PSCustomObject]@{
    schema_version = 1
    action = "memory-mechanism-intervene"
    dataset = $DatasetName
    corpus = $Corpus
