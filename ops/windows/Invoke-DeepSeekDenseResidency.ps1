...
$bundle = Join-Path (Join-Path $script:RepoRoot "work") `
    "deepseek-dense-set-${stamp}"
$executable = Join-Path $script:RepoRoot `
    "out\build\windows-msvc-release\runtime\Release\expert-deepseek-dense-residency.exe"
if (-not (Test-Path $executable -PathType Leaf)) {
    throw "Build the CUDA runtime before running dense residency"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        throw "DeepSeek dense residency failed"
    }
    ...
    $artifact = Write-JsonArtifact -Value $result `
        -Name "deepseek-dense-residency-latest.json"
    $nativeRaw.Trim()
    Write-Output "Dense residency artifact: $artifact"
}
finally {
    Pop-Location
}
