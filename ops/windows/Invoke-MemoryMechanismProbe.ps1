param(
    ...
    [string]$OutputName = "memory-mechanism-probe-v1",
    [int]$InDistributionCases = 4,
...
if ($DatasetName -notmatch '^[A-Za-z0-9._-]+$' -or
    ...
    throw "Invalid Memory Mechanism Probe parameters"
}
...
$output = Join-Path $outputRoot "probe.json"
...
foreach ($required in @(
    ...
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Memory Mechanism Probe artifact is missing: $required"
    }
}
...
if ($freeVramMiB -lt $MinimumFreeVramMiB) {
    throw "Memory Mechanism Probe requires $MinimumFreeVramMiB MiB free VRAM; available=$freeVramMiB"
}
